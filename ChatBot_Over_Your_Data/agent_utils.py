...
class RAGParams(BaseModel):
    ...
    Parameters used to configure a AI2U pipeline.
    ...
    include_summarization: bool = Field(
        default=False,
        description=(
            "Whether to include summarization in the AI2U pipeline. (only for gpt-3.5-turbo)"
        ),
    )
    top_k: int = Field(
...
class ParamCache(BaseModel):
    ...
    system_prompt: Optional[str] = Field(
        default=None, description="System prompt for AI2U agent."
    )
    # data
    ...
    docs: List = Field(default_factory=list, description="Documents for AI2U agent.")
    # tools
    tools: List = Field(
        default_factory=list, description="Additional tools for AI2U agent (e.g. web)"
    )
    # RAG params
    rag_params: RAGParams = Field(
        default_factory=RAGParams, description="AI2U parameters for AI2U agent."
    )

    # agent params
    vector_index: Optional[VectorStoreIndex] = Field(
        default=None, description="Vector index for AI2U agent."
    )
    agent_id: str = Field(
        default_factory=lambda: f"Agent_{str(uuid.uuid4())}",
        description="Agent ID for AI2U agent.",
    )
    agent: Optional[BaseChatEngine] = Field(default=None, description="AI2U agent.")
...
class RAGAgentBuilder:
    ...
    def get_rag_params(self) -> Dict:
        """Get parameters used to configure the AI2U pipeline.
    ...
    def set_rag_params(self, **rag_params: Dict) -> str:
        ...
        Args:
            **rag_params (Dict): dictionary of AI2U parameters.
        ...
        return "AI2U parameters set successfully."
...
4) Set parameters for the AI2U pipeline.
5) Build the agent
